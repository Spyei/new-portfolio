...
export default function DiscordActivity({
	...
	const { data, status } = useDiscordActivity({ userId: DISCORD_USER_ID });
	const { t } = useI18n();
	...
	return (
		<motion.div
			{...springElement(0.9, firstVisitDelay(3.1, 2.1, isFirstVisit))}
			className="flex flex-col gap-2"
		>
			<Title>{t.discord.title}</Title>
			<span>{t.discord.description}</span>
			<motion.div
				{...springElement(0.9, firstVisitDelay(3.3, 2.3, isFirstVisit))}
			...
			>
				...
				<div className="flex flex-col gap-3">
					{status === "connecting" && !data && <SkeletonCard />}
						...
						<>
							{visibleActivities.map((activity) => (
								<motion.div
									{...springElement(
										0.9,
										firstVisitDelay(2.8, 1.8, isFirstVisit),
									)}
									key={activity.id}
								>
									<ActivityCard
							...
							{data.listening_to_spotify && data.spotify && (
								<motion.div
									{...springElement(
										0.9,
										firstVisitDelay(2.9, 1.9, isFirstVisit),
									)}
									key={1}
								>
									<SpotifyCard
